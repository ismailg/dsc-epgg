...
def _group_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        grouped[(str(row.get("condition", "")), ablation, int(checkpoint_episode), key)].append(row)
    ...
    for (condition, ablation, episode, f_value), cur in sorted(grouped.items()):
        ...
        for row in cur:
            seed_rows[int(float(row.get("train_seed", "0") or 0))].append(row)
        coop_by_seed = [_mean(float(row["coop_rate"]) for row in rows) for rows in seed_rows.values()]
        reward_by_seed = [_mean(float(row["avg_reward"]) for row in rows) for rows in seed_rows.values()]
        welfare_by_seed = [_mean(float(row[avg_welfare_key]) for row in rows) for rows in seed_rows.values()]
        out.append(
            {
                "condition": condition,
                "ablation": ablation,
