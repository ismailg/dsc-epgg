...
IWR_RUN_DIR="${IWR_RUN_DIR:-/export/scratch/iguennou/runs/dsc-epgg-vectorized/phase3-vecstraight-qx6-100k-sender-shuffle-vecparity-20260327}"
NEXT_ROOT="${NEXT_ROOT:-${IWR_PROJECT_DIR}/outputs/eval/phase3_vecstraight_next_steps_${RUN_DATE}}"
MANIFEST_DIR="${MANIFEST_DIR:-${NEXT_ROOT}/manifests_iwr_${BRANCH_EP}_${MODE}_vecparity}"
...
log() {
  printf '[%s] [%s-%s] %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "${BRANCH_EP}" "${MODE}" "$*" | tee -a "${PROGRESS_LOG}"
}
...
log "starting vectorized continuation branch=${BRANCH_EP} mode=${MODE}"
NEXT_ROOT="${NEXT_ROOT}" \
COMM_MANIFEST="${COMM_MANIFEST}" \
BASELINE_MANIFEST="${BASELINE_MANIFEST}" \
RUN_DATE="${RUN_DATE}" \
...
./scripts/run_phase3_vecstraight_continuation_mode.sh "${BRANCH_EP}" "${MODE}" iwr | tee -a "${PROGRESS_LOG}"

log "completed vectorized continuation branch=${BRANCH_EP} mode=${MODE}"
