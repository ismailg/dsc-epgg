...
JOB_DIR="${RUN_ROOT}/jobs/post_eval_pipeline_local"
mkdir -p "${JOB_DIR}"
...
"${PYTHON_BIN}" scripts/launch_detached_local.py \
  ...
  --env "RUN_KIND=local" \
  -- /bin/bash "${REPO_ROOT}/scripts/run_phase3_vecstraight_post_eval_pipeline.sh" local
...
"${PYTHON_BIN}" scripts/launch_detached_local.py \
  ...
  -- "${PYTHON_BIN}" "${REPO_ROOT}/scripts/progress_watch.py" \
    ...
    --mode regex-file \
    --source "${PIPE_STDOUT}" \
    --pattern "\\[post-eval-pipeline\\] progress current=(?P<current>[0-9]+) total=(?P<total>[0-9]+)" \
    --state-json "${WATCH_STATE}" \
    --events-jsonl "${WATCH_EVENTS}" \
    --label "phase3_vecstraight_post_eval_pipeline_local_${RUN_DATE}" \
    --poll-seconds 20
