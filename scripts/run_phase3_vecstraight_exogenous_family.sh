...
case "${MODE}" in
  fixed0|fixed1|public_random|uniform) ;;
  *)
    echo "unsupported MODE=${MODE}; expected one of: fixed0 fixed1 public_random uniform" >&2
    exit 2
    ;;
esac
...
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/outputs/eval/phase3_vecstraight_exogenous_${MODE}_15seeds_${RUN_KIND}_${RUN_DATE}}"
...
printf '[phase3-exogenous] mode=%s out_root=%s\n' "${MODE}" "${OUT_ROOT}"
