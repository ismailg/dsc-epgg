...
if [[ $# -lt 1 ]]; then
  echo "usage: $0 MILESTONE [RUN_KIND]" >&2
  exit 2
fi
...
case "${MILESTONE}" in
  50000) LABEL="frozen50k" ;;
  150000) LABEL="frozen150k" ;;
  *)
...
"${PYTHON_BIN}" -m src.analysis.validate_checkpoint_suite_outputs \
  ...
  --expected-interventions none zeros fixed0 fixed1 indep_random public_random sender_shuffle permute_slots
