...
Every quantitative claim in main.qmd traces back to a function or attribute
in this module.  Run ``python paper_data.py`` to execute self-checks.
"""
...
INTERVENTION_CSV = DATA_ROOT / "phase3_annealed_ext150k_5seeds" / "report" / "intervention_delta_table.csv"
RECV_SEMANTICS_CSV = DATA_ROOT / "phase3_annealed_ext150k_5seeds" / "report" / "receiver_semantics_summary.csv"
RECV_BY_SENDER_CSV = DATA_ROOT / "phase3_annealed_ext150k_5seeds" / "report" / "receiver_by_sender_summary.csv"
ALIGNMENT_CSV = DATA_ROOT / "phase3_annealed_ext150k_5seeds" / "report" / "sender_alignment_summary.csv"
...
# Same-checkpoint continuation (may not exist yet)
SAMECKPT_DIR = DATA_ROOT / "phase3_sameckpt_continuation_5seeds"
...
def _bootstrap_paired_delta(
    ...
    """Return (delta_mean, ci_lower, ci_upper, two-sided p_value)."""
    rng = random.Random(seed)
    samples = sorted(
        _mean(rng.choices(values, k=len(values))) for _ in range(n_boot)
    )
...
class GapCell:
    ...
    def __post_init__(self):
        self.mean, self.ci_lo, self.ci_hi, self.p_value = (
            _bootstrap_paired_delta(self.deltas)
        )
...
def load_table3() -> List[Table3Row]:
    ...
    Uses phase3_channel_controls_ext150k_3seeds data.
    If phase3_sameckpt_continuation_5seeds has data, uses that instead.
    """
...
def table3_is_sameckpt() -> bool:
    """True if Table 3 data comes from the 5-seed same-checkpoint experiment."""
    sameckpt_csv = SAMECKPT_DIR / "report" / "channel_control_summary.csv"
...
def compute_prose_claims(t1: Table1, t2: List[ControlRow],
    ...
    p.comm_gap_35_50k_pval = f"{g35_50.p_value:.3f}"
    p.comm_gap_35_50k_stars = _stars(g35_50.p_value)
...
def generate_defs_tex(d: PaperData) -> str:
    ...
    for fv, fl in fv_labels:
        for ep, el in ep_labels:
            ...
            if fv == "3.500" and ep == 50_000:
                gap_str = f"\\mathbf{{{fmt_delta(g.mean)}}}{s}"
            else:
                gap_str = f"{fmt_delta(g.mean)}{s}"
    ...
    t3_pending = "" if d.t3_is_sameckpt else \
        " [Pending: 5-seed same-checkpoint replication.]"
    ...
    for row in d.t3:
        ...
        D(f"TIII{ml}Fk", fmt(row.ep50k), f"{row.mode} 50k f=3.5")
        hk_val = fmt(row.ep100k) if row.ep100k is not None else "---"
        D(f"TIII{ml}Hk", hk_val, f"{row.mode} 100k f=3.5")
        D(f"TIII{ml}Ofk", fmt(row.ep150k), f"{row.mode} 150k f=3.5")
        # f=5.0 values for difficulty-gating table
        if not math.isnan(row.ep50k_f50):
            D(f"TIII{ml}FkFV", fmt(row.ep50k_f50), f"{row.mode} 50k f=5.0")
        if not math.isnan(row.ep150k_f50):
            D(f"TIII{ml}OfkFV", fmt(row.ep150k_f50), f"{row.mode} 150k f=5.0")
    ...
    # --- Table 4: mute experiment (6 cells) ---
    lines.append("")
    lines.append("% Table 4: mute experiment")
    ...
    # --- Table 5: aggregate vs per-sender (9 cells + 3 ratios) ---
    lines.append("")
    lines.append("% Table 5: aggregate vs per-sender token effects")
    ...
    # --- Table 6: polarity alignment (5 seeds) ---
    lines.append("")
    lines.append("% Table 6: polarity alignment")
    seed_labels = {101: "A", 202: "B", 303: "C", 404: "D", 505: "E"}
    for row in d.t6:
        sl = seed_labels[row.seed]
        D(f"TVIpattFk{sl}", f"{row.ep50k_pos}:{row.ep50k_neg}",
          f"seed {row.seed} 50k")
        D(f"TVIpattOfk{sl}", f"{row.ep150k_pos}:{row.ep150k_neg}",
          f"seed {row.seed} 150k")
        D(f"TVItraj{sl}", row.trajectory, f"seed {row.seed}")
    ...
    D("pvGapTFFkPval",
      "<0.001" if d.t1.gap[("3.500", 50_000)].p_value < 0.001
      else f"={fmt(d.t1.gap[('3.500', 50_000)].p_value)}",
    ...
    # --- Appendix: per-seed values ---
    # Use letter labels instead of digits (TeX \def names must be all-alpha)
    lines.append("")
    lines.append("% Appendix: per-seed cooperation and gaps at f=3.5")
    seeds = d.t1.comm[("3.500", 50_000)].seeds
    per_seed_labels = {101: "Sa", 202: "Sb", 303: "Sc", 404: "Sd", 505: "Se"}
    for s in seeds:
        sl = per_seed_labels[s]
        for ep, el in ep_labels:
            cell = d.t1.comm[("3.500", ep)]
            idx = cell.seeds.index(s)
            D(f"pvPScomm{sl}{el}", fmt(cell.values[idx]),
              f"seed {s} comm 3.5 {ep//1000}k")

            gap_val = d.t1.per_seed_gap.get((s, "3.500", ep), float("nan"))
            D(f"pvPSgap{sl}{el}", fmt_delta(gap_val),
              f"seed {s} gap 3.5 {ep//1000}k")

    # Mean values for appendix bottom rows
    for ep, el in ep_labels:
...
def _check():
        ...
        for ep in (50_000, 100_000, 150_000):
            ...
            if c and nc and g:
                print(f"  f={fv} ep={ep//1000}k: "
                      ...
                      f"gap={fmt_delta(g.mean)} p={g.p_value:.3f}{_stars(g.p_value)}")
    ...
    print("\n=== Table 5: aggregate vs per-sender ===")
    for row in d.t5:
    ...
    print(f"  Comm gap f=3.5 50k: +{p.comm_gap_35_50k_pp}pp "
          f"(p={p.comm_gap_35_50k_pval}{p.comm_gap_35_50k_stars})")
    print(f"  Gap range at 50k: +{p.gap_range_lo} to +{p.gap_range_hi}pp")
    print(f"  At 150k: {p.n_positive_150k}/5 positive, seed {p.negative_seed} negative")
